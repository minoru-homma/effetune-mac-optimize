...
const pluginPath = path.join(repoRoot, 'plugins', 'dynamics', 'auto_leveler.js');
const wasmPath = path.join(repoRoot, 'plugins', 'wasm', 'auto_leveler.wasm');
...
const params = {
  ...
  tg: -16, tw: 1500, mg: 4, ng: -12, at: 50, rt: 1250, gt: -60
};
...
for (let blk = 0; blk < BLOCKS; blk++) {
  ...
  fillInput(data, blk);
  const out = jsRun(jsCtx, data, params, blk * BLOCK / SR);
...
(async () => {
  ...
  e.set_params(sp, params.tg, params.tw, params.mg, params.ng, params.at, params.rt, params.gt);
